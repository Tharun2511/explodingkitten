...
import { checkEmail, toastError, toastSuccess } from "../../helpers/Helper";
...
const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!userDetails.firstname) {
            toastError("Firstname cannot be empty");
            return;
        } else if (userDetails.firstname.length < 3) {
            toastError("Firstname must be atleast 3 characters");
            return;
        } else if (!userDetails.firstname.length > 8) {
            toastError("Firstname cannot be more than 8 characters");
            return;
        } else if (!userDetails.lastname) {
            toastError("Lastname cannot be empty");
            return;
        } else if (userDetails.lastname.length > 12) {
            toastError("Lastname cannot be more than 12 characters");
            return;
        } else if (!userDetails.email) {
            toastError("Email cannot be empty");
            return;
        } else if (!checkEmail(userDetails.email)) {
            toastError("Enter a valid Email ID");
            return;
        } else if (!userDetails.gender) {
            toastError("Please choose a gender");
        } else if (
            !userDetails.dob ||
            new Date(userDetails.dob) > new Date() ||
            new Date(userDetails.dob) < new Date("1900-01-01")
        ) {
            toastError("Enter a valid Date of Birth");
